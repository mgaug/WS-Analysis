...
has_multiple_data_spacings = config['has_multiple_data_spacings']
...
coverage_cut = config['coverage_cut']
coverage_cut_for_daily_samples = config['coverage_cut_for_daily_samples']
coverage_cut_for_monthly_samples = config['coverage_cut_for_monthly_samples']
day_coverage_for_samples = config['day_coverage_for_samples']
day_coverage_for_diurnal = config['day_coverage_for_diurnal']

wind_expr = config['wind_binning']
wind_binning = eval(wind_expr, {"np": np})

print ('wind binning: ', wind_binning)

WS_start      = config['WS_start']
WS_end        = config['WS_end']
WS_relocation = config['WS_relocation']
new_WS        = config['new_WS']
new_model     = config['new_model']
old_model     = config['old_model']
NOT_end_of_5min = config['NOT_end_of_5min']
NOT_end_of_data = config['NOT_end_of_data']

#WS_start_d = datetime.strptime(WS_start, "%Y-%m-%d %H:%M:%S")
#WS_end_d = datetime.strptime(WS_end, "%Y-%m-%d %H:%M:%S")
WS_start_y = datetime(WS_start.year,1,1)

ynums = np.array([mdates.date2num(WS_start_y)])
for y in np.arange(WS_start.year,WS_end.year+1):
    ynums = np.concatenate((ynums,np.array([mdates.date2num(datetime(y,1,1))])))

print('ynums: ', ynums)
    
CTAOS_start_date = config['CTAOS_start_date']
CTAOS_end_date   = config['CTAOS_end_date']  

str_NOT_interval = config['str_NOT_interval']
...
name_mu_sq           = ['A', 'B', 'Q', 'C', 'phi_mu',     'sigma0' ]
...
bounds_mu_sq           = ((0, None), (None, None), (None, None), (0, None), (0, None), (0.1,None))
...
def plot_downtime() -> None:
    ...
    dfff, coverage = apply_coverage(dfff,data_spacing_minutes,debug=False)
    ...
    plot_mensual_downtime(dfff,cuts_dict,data_spacing_minutes,coverage_cut=80)
    plt.savefig('Downtime_mensual{:s}.pdf'.format(tits),bbox_inches='tight')
    plt.show()

    cuts_dict = { name_humidity : [ '<', 90, '%' ], name_ws_average : [ '<', 36, 'km/h'] }

    cmap = plt.get_cmap("tab10")
    plt.figure(figsize = (10,5), constrained_layout = True)
    wg_limit = 60.
    waiting_time = 20
    plot_downtime_vs_windaverage(dfff,cuts_dict,name_ws_average,20.,50.,data_spacing_minutes,coverage_cut=80,windgust_limit=wg_limit,windgust_waitingtime=waiting_time, color=cmap(0))
    wg_limit = 50.
    waiting_time = 20
    plot_downtime_vs_windaverage(dfff,cuts_dict,name_ws_average,20.,50.,data_spacing_minutes,coverage_cut=80,windgust_limit=wg_limit,windgust_waitingtime=waiting_time, color=cmap(1))
    wg_limit = 40.
    waiting_time = 20
    plot_downtime_vs_windaverage(dfff,cuts_dict,name_ws_average,20.,50.,coverage_cut=80,windgust_limit=wg_limit,windgust_waitingtime=waiting_time, color=cmap(2))
    plt.xlabel('Maximum wind speed average (km/h)', fontsize=26)
    plt.ylabel('Mean downtime (%)',fontsize=26)
    plt.legend(loc='best')
    plt.savefig('Downtime_windaverage_WaitTime{:.0f}{:s}.pdf'.format(waiting_time,tits),bbox_inches='tight')
    plt.show()
    ...
    wg_limit = 40.
    waiting_time = 10
    plot_downtime_vs_windaverage(dfff,cuts_dict,name_ws_average,20.,50.,data_spacing_minutes,coverage_cut=80,windgust_limit=wg_limit,windgust_waitingtime=waiting_time, color=cmap(2))
    plt.xlabel('Maximum wind speed average (km/h)', fontsize=26)
    plt.ylabel('Mean downtime (%)',fontsize=26)
    plt.legend(loc='best')
    plt.savefig('Downtime_windaverage_WaitTime{:.0f}{:s}.pdf'.format(waiting_time,tits),bbox_inches='tight')
    ...
    cmap = plt.get_cmap("tab10")
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_downtime_vs_windgust(dfff,cuts_dict,36.,70.,data_spacing_minutes,coverage_cut=80,windgust_waitingtime=10, color=cmap(0))
    plot_downtime_vs_windgust(dfff,cuts_dict,36.,70.,data_spacing_minutes,coverage_cut=80,windgust_waitingtime=20, color=cmap(1))
    plot_downtime_vs_windgust(dfff,cuts_dict,36.,70.,data_spacing_minutes,coverage_cut=80,windgust_waitingtime=30, color=cmap(2))
    plt.xlabel('Maximum wind gust (km/h)', fontsize=26)
    plt.ylabel('Mean downtime (%)',fontsize=26)
    plt.legend(loc='best')
    plt.savefig('Downtime_windgust{:s}.pdf'.format(tits),bbox_inches='tight')
    plt.show()
...
def plot_temperature_gradient() -> None:
    ...
    gradients = [ 10, 30 ]   # 20 min and 60 min
    ...
    for i in gradients:
        ...
        xtit = 'Temperature change rate (ºC/20 min)'
        ytit = 'Probability / (ºC/min)'
        unit = 'ºC/min'
        mult = 20.
        res  = 1./20

        if i==30:
            xtit = 'Temperature change rate (ºC/h)'
            ytit = 'Probability / (ºC/min)'
            unit = 'ºC/h'
            mult = 60.
            res  = 1./60
        ...
        print ('var=',var)
        print ('dfff=',dfff[var].head(n=100))
        print ('dfff=',dfff[var].tail(n=100))        
        
        dffff = dfff[dfff[var].notnull()]
        print ('dffff=',dffff.head(n=100))
        print ('dffff=',dffff.tail(n=100))
        
        #Distribucions anuals i mensuals:
        plt.clf()
        plot_hist(dffff, var, 0.01,xtit,ytit,unit, xoff=0.08, coverage_cut=coverage_cut,mult=mult)
        #plt.plot(x,y, color= 'steelblue')
        plt.savefig(plotdir+'/Hist_'+var+tits+'.pdf', bbox_inches='tight')
        plt.show()
        
        plt.clf()
        plot_hist(dffff, var, 0.01,xtit,ytit,unit, xoff=0.08, is_night=True, coverage_cut=coverage_cut,mult=mult)
        #plt.plot(x,y, color= 'steelblue')
        plt.savefig(plotdir+'/Hist_'+var+tits+'_night.pdf', bbox_inches='tight')
        plt.show()
        
        plt.clf()
        plot_mensual(dffff, var,xtit,coverage_cut=coverage_cut,mult=mult)
        plt.savefig(plotdir+'/'+var+tits+'_mensual.pdf', bbox_inches='tight')
        plt.show()
        
        plt.figure(figsize = (10,5), constrained_layout = True)
        plot_historic(dffff, var, coverage, ynums, min_coverage=coverage_cut)
        #plt.xlabel('Year')
        plt.ylabel(xtit,fontsize=18)
        plt.savefig(plotdir+'/'+var+tits+'_sencer.pdf', bbox_inches='tight')
        plt.show()
        ...
        plt.figure(figsize = (10,5), constrained_layout = True)
        plot_historic(dffff, var, coverage, ynums, min_coverage=coverage_cut, mult=mult)
        #plt.xlabel('Year')
        plt.ylabel(xtit,fontsize=18)
        plt.savefig(plotdir+'/'+var+tits+'_sencer.pdf', bbox_inches='tight')
        plt.show()
    ...
    plt.figure(figsize = (10,5), constrained_layout = True)
    h,p = plot_profile(dfff[name_pressure],dfff['Tgradient5R'],nbins=50)
    plt.xlabel('Pressure (mbar)')
    plt.ylabel('Temperature change rate (ºC/min)',fontsize=22)
    plt.savefig('{:s}/Tgradient5R_corr_pressure{:s}.pdf'.format(plotdir,tits))
    ...
    plt.savefig('{:s}/Tgradient10R_corr_Rgradient10R_shift0T{:s}.pdf'.format(plotdir,tits))
    ...
    for s in np.arange(2,15,1):
        ...
        plt.xlabel('RH change rate (%/min)')
        plt.ylabel(r'Temperature change rate (ºC/min)',fontsize=22)
        plt.savefig('{:s}/Tgradient10R_corr_Rgradient10R_shift{:d}T{:s}.pdf'.format(plotdir,s,tits))
    
    for s in np.arange(2,120,10):
        plt.figure(figsize = (10,5), constrained_layout = True)
        #print ('TEST SHIFT ', s,': ', dfff_p['Rgradient5R'].shift(s,freq='T').head(n=20))
        h,p = plot_profile(dfff[name_pressure].shift(s,freq='T'),dfff['Tgradient10R'],nbins=50, is_shifted=True)
        plt.xlabel('pressure (mbar)')
        plt.ylabel(r'Temperature change rate (ºC/min)',fontsize=22)
        plt.savefig('{:s}/Tgradient10R_corr_pressure_shift{:d}T{:s}.pdf'.format(plotdir,s,tits))
    ...
    for s in np.arange(2,120,10):
        ...
        plt.savefig('{:s}/Tgradient10R_corr_Pgradient5R_shift{:d}T{:s}.pdf'.format(plotdir,s,tits))

def plot_DTR() -> None:

    #plt.rcParams.update(params)

    is_offset = config['is_DTR_offset']    
    is_averaging = config['is_DTR_averaging']
    is_naoi = config['is_naoi']
    is_NOT = config['is_DTR_NOT']
    is_humcorr = config['is_DTR_humcorr']
    is_dCm = config['is_DTR_dCm']
    is_profile2d = config['is_DTR_profile2d']    
    avg_n = config['avg_n_DTR']
    freq_n = config['freq_n_DTR']
    #tits = '_5avg_r'
    #tits = '_Paranal'
    tits = config['tits']    
    offset_from_temperature = config['offset_from_temperature']   # result of offset analysis from plot-temperature

    if has_multiple_data_spacings:        
        mask_before = ((dff.index < config['WS_dataspacing_change']) & (dff[name_temperature]>config['minimum_temperature']) & (dff[name_temperature]<config['maximum_temperature']))
        mask_after = ((dff.index > config['WS_dataspacing_change']) & (dff[name_temperature]>config['minimum_temperature']) & (dff[name_temperature]<config['maximum_temperature']))

        dfff_before, coverage_before = apply_coverage(dff[mask_before],data_spacing=config['data_spacing_before'],debug=False)
        dfff_after, coverage_after = apply_coverage(dff[mask_after],data_spacing=config['data_spacing_after'],debug=False)

        dfff = pd.concat([dfff_before, dfff_after])
        coverage = pd.concat([coverage_before, coverage_after])
    else:    
        dfff = dff[(dff['temperature_reliable']==True)]
        dfff, coverage = apply_coverage(dfff,data_spacing_minutes,debug=False)
    ...
    if has_multiple_data_spacings:        
        df_s1  = dfn[mask_before].shift(8,freq='H')  # calculate spread from 8:00 to 8:00
        df_s1  = df_s1[df_s1['coverage']>coverage_cut_for_daily_samples]

        expected_data = 60*24/config['data_spacing_before']        
        mask_daily = (df_s1['mjd'].resample('D').count().dropna() > day_coverage_for_diurnal/100*expected_data)

        if is_averaging: 
            diu_s1, mjd_s1, hum_s1 = average_diurnal_df_rolling(df_s1,mask_daily,name_temperature,name_humidity,freq=config['freq_n_DTR_before'])
        else:
        ...
        expected_data = 60*24/config['data_spacing_after']
        mask_daily = (df_s2['mjd'].resample('D').count().dropna() > day_coverage_for_diurnal/100*expected_data)

        if is_averaging: 
            diu_s2, mjd_s2, hum_s2 = average_diurnal_df_rolling(df_s2,mask_daily,name_temperature,name_humidity,freq=config['freq_n_DTR_before'])
        else:
    ...
    init_diu_DTR = config['init_DTR']
    init_diu = eval(init_diu_DTR, {"np": np})        
    init_diu_DTR_nob = config['init_DTR_nob']
    init_diu_nob = eval(init_diu_DTR_nob, {"np": np})        
    init_diu_DTR_dphim = config['init_DTR_dphim']
    init_diu_dphim = eval(init_diu_DTR_dphim, {"np": np})
    if is_dCm:
        init_diu_DTR_dCm  = config['init_DTR_dCm']
        init_diu_dCm = eval(init_diu_DTR_dCm, {"np": np})            
    init_diu_DTR_offset = config['init_DTR_offset']
    init_diu_offset = eval(init_diu_DTR_offset, {"np": np})            
    ...
    plt.savefig('{:s}/DiurnalTemperature_corr_d_humidity{:s}.pdf'.format(plotdir,tits))
    ...
    if is_dCm:    
        ...
        plt.xlabel('Relative humidity (%)')
        plt.ylabel(r'DTR fit residuals')
        plt.savefig('{:s}/DiurnalTemperature_corr_dCm_humidity{:s}.pdf'.format(plotdir,tits))

        print ('HUM FIT dCm RES.:',popt)
        hum_dCm_p0 = popt[0]
        hum_dCm_p1 = popt[1]

    if is_humcorr and is_dCm:
    ...
    if is_NOT:
        diu_lik_dCm_NOT_daily = Likelihood_Wrapper(loglike_dCm,mu_dCm,
                                                   name_mu_dCm,init_diu_dCm,bounds_mu_dCm,
                                                   'Eq. (4), {:s}'.format(str_NOT_interval))    
        ...
        diu_lik_dCm_NOT = Likelihood_Wrapper(loglike_dCm,mu_dCm,
                                             name_mu_dCm,init_diu_dCm,bounds_mu_dCm,
                                             'Eq. (4), monthly means {:s}'.format(str_NOT_interval))    
    ...
    if is_dCm:
        ...
        plt.xlabel('Relative humidity (%)')
        plt.ylabel(r'DTR fit residuals')
        plt.savefig('{:s}/DiurnalTemperature_corr_dCm_humidity_hum{:s}.pdf'.format(plotdir,tits))
    ...
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_diurnal_spread(dfff, name_temperature, coverage, ynums, expected_data_per_day)
    ...
    if is_humcorr and is_dCm:
        ...
        plot_diurnal_fit_results(mjd_s,diu_lik_daily,is_daily=True,color='r')    
    ...
    plt.savefig('{:s}/Temperaturadiurnal_sencer{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    tits = tits.replace('_offset','')

    if is_dCm and is_profile2d:
        ...
        ax.set_ylim(0,1.0)
        plt.savefig('{:s}/DiurnalTemperature_profiled_daily_contour{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')

    #2D profile of diurnal temperature increase and amplitude increase
    if is_humcorr and is_dCm and is_profile2d:
        plt.figure(figsize = (8,7), constrained_layout = True)
        plt.xlabel(r'$b$ (DTR increase) (ºC/10y)', fontsize = 25)
        plt.ylabel(r'$\Delta C_{m}$ (DTR seasonal ampltiude increase) (ºC/10y)', fontsize = 22)
        ...
        ax.set_xlim(-0.05,0.3)
        ax.set_ylim(0,1.0)
        plt.savefig('{:s}/DiurnalTemperature_profiled_dailyhum_contour{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    ...
    if is_dCm:
        ...
        plt.savefig('{:s}/DiurnalTemperatureSeasonal_b_profiled{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
        tits = tits.replace('_offset','')
    ...
    if is_offset:
        ...
        tits = tits + '_offset'                
    plt.savefig('{:s}/DiurnalTemperature_b_profiled{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    tits = tits.replace('_offset','')
...
def plot_datacount() -> None:
    ...
    if 'Paranal' not in h5file_long and 'CTAO' not in h5file_long:
    ...
    ax.set_xlim([0.,100.])
    plt.savefig('Data_Count{:s}.pdf'.format(tits))
    plt.show()
...
def plot_temperature() -> None:
    ...
    is_sunanomaly = False
    is_hessian = config['is_temperature_hessian']
    is_naoi = config['is_naoi']
    is_offset = config['is_temperature_offset']
    is_squared = config['is_temperature_squared']
    is_coauthor_tests = config['is_coauthor_tests']
    is_not = config['is_temperature_NOT']
    tits = config['tits']
    ...
    if has_multiple_data_spacings:        
        ...
        dfff = dff[((dff['temperature_reliable']==True) & (dff[name_temperature]<config['maximum_temperature']) & (dff[name_temperature]>config['minimum_temperature']))]
        dfff, coverage = apply_coverage(dfff,data_spacing_minutes,debug=False)

    dfff_p = dfff[((dfff['pressure_reliable']==True) & (dfff['Pgradient0']<2) & (dfff[name_pressure]<config['maximum_pressure']) & (dfff[name_pressure]>config['minimum_pressure']) & (dfff['Pgradient0']>-2))]
    
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_mensual(dfff, name_temperature, 'Temperature (ºC)', coverage_cut=coverage_cut, fullfits=True)
    plt.savefig('{:s}/Temperatura_mensual{:s}.pdf'.format(plotdir,tits))
    plt.show()
    
    plt.clf()
    plot_mensual_distributions(dfff, name_temperature, 'Temperature - monthly mean (ºC)', 0.1)
    plt.savefig('{:s}/Temperatura_distributions{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    init_temp = eval(init_temperature, {"np": np})        
    init_temperature_sq = config['init_temperature_squared'] 
    init_temp_sq = eval(init_temperature_sq, {"np": np})        
    init_temperature_dphim = config['init_temperature_dphim'] 
    init_temp_dphim = eval(init_temperature_dphim, {"np": np})        
    init_temperature_dCm  = config['init_temperature_dCm'] 
    init_temp_dCm  = eval(init_temperature_dCm, {"np": np})        
    init_temperature_Haslebacher = config['init_temperature_monthly'] 
    init_temp_Haslebacher = eval(init_temperature_Haslebacher, {"np": np})        
    init_temperature_offset = config['init_temperature_offset']
    init_temp_offset = eval(init_temperature_offset, {"np": np})        
    ...
    temp_lik_mean.setargs_df(dfn,name_temperature,mask,
                             ...
                             data_expected=expected_data_per_day)
    ...
    if is_squared:
        temp_lik_median_sq = Likelihood_Wrapper(loglike_sq,mu_sq,
                                                name_mu_sq,init_temp_sq,bounds_mu_sq,
                                                'Eq. (2) w/quad. term, daily medians')
        temp_lik_median_sq.setargs_df(dfn,name_temperature,mask,
                                   is_daily=True,is_median=True,day_coverage=day_coverage_for_samples,
                                   data_expected=expected_data_per_day)    
    ...
    if is_not:        
        temp_lik_median_NOT = Likelihood_Wrapper(loglike,mu,
                                                 name_mu,init_temp,bounds_mu,
                                                 'Eq. (2), daily medians, {:s}'.format(str_NOT_interval))
    ...
    if is_squared:
        temp_lik_median_sq.like_minimize(method=method,tol=tol)        
    ...
    if is_sunanomaly:
        ...
        plt.savefig('SunAz_Temperature{:s}.pdf'.format(tits))
        ...
        #plt.plot(dfn.loc[mask_sun,'sun_az'],dfn.loc[mask_sun,name_temperature],'.',color='r')
        plt.savefig('SunAlt_Temperature{:s}.pdf'.format(tits))
        ...
        #plt.plot(dfn.loc[mask_sun,'sun_az'],dfn.loc[mask_sun,name_temperature],'.',color='r')
        plt.savefig('SunAltAz_Temperature{:s}.pdf'.format(tits))
    
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_historic(dfff, name_temperature, coverage, ynums, min_coverage=coverage_cut)
    #plt.xlabel('Year')
    plt.ylabel('Temperature (ºC)', fontsize=18)
    if is_offset:
        plot_historic_fit_results(dfn,mask1,temp_lik_median_offset,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_samples,
                                  color='red',is_sigma2=False,is_offset=True)
        plot_historic_fit_results(dfn,mask2,temp_lik_median_offset,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_samples,
                                  color='orange',is_sigma2=False,is_offset=True,offset=temp_lik_median_offset.res.x[-1])
        tits = tits + '_offset'        
    else:
        plot_historic_fit_results(dfn,mask,temp_lik_median,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_samples)
    if is_squared:
        plot_historic_fit_results(dfn,mask,temp_lik_median_sq,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_samples, color='forestgreen')        
    #plot_historic_fit_results(dfn_H,mask_H,temp_lik_Haslebacher,expected_data_per_day,is_daily=False,color='orange')    
    plt.savefig('{:s}/Temperatura_sencer{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('{:s}/Temperatura_residuals{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('{:s}/Temperatura_residuals_hist{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    ...
    temp_lik_median_rob.setargs_df(dfn,name_temperature,mask,
                                   ...
                                   day_coverage=day_coverage_for_samples,
                                   data_expected=expected_data_per_day,np_mask=mask_chi2)    
    ...
    if is_offset:
        ...
        tits = tits + '_offset'
    if is_squared:
        temp_lik_median_sq.profile_likelihood(1,chi2=chi2,NN=NN,method=method,col='forestgreen',alpha=0.2)
        tits = tits + '_squared'        
    if is_coauthor_tests:
    ...
    plt.savefig('{:s}/Temperature_b_profiled{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    if is_offset:
        tits = tits.replace('_offset','')
    if is_coauthor_tests:
        tits = tits.replace('_coauthors','')
    ...
    plt.savefig('{:s}/Histogram_temperature{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.xticks(fontsize = 25)
    plt.yticks(fontsize = 25)
    plt.savefig('{:s}/Histogram_logtemperature{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    #plt.plot(x,y, color= 'steelblue')
    plt.savefig('{:s}/Hist_temperature{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()

    plt.clf()
    plot_hist(dfff, name_temperature, 1.,'Temperature (ºC)','Probability / ºC', 'ºC', xoff=0.08, is_night=True, coverage_cut=coverage_cut)
    #plt.plot(x,y, color= 'steelblue')
    plt.savefig('{:s}/Hist_temperature_night{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
...
def plot_DP() -> None:
    ...
    plt.legend(loc='best')
    plt.savefig('{:s}/DP_vs_Humidity_corr{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')
    ...
    plt.ylabel('Temperature minus dew point (ºC)',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/DP_vs_HighHumidity_corr{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')
    ...
    plt.legend(loc='best')
    plt.savefig('{:s}/Humidity_vs_DP_corr{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')
    ...
    plt.xlabel('Temperature minus dew point (ºC)',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Humidity_vs_LowDP_corr{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')
...
def plot_humidity() -> None:
    ...
    is_offset = config['is_humidity_offset']
    is_coauthor_tests = config['is_coauthor_tests']
    is_20to22 = config['is_humidity_20to22']
    is_hessian = config['is_humidity_hessian']
    is_not = config['is_humidity_NOT']
    ...
    if is_not:
        ...
        plt.savefig('{:s}/Humidity_corr_NOT{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

        plt.figure(figsize = (10,5), constrained_layout = True)
        not_profile_time(df_not,dff,'Humidity',name_humidity,expected_data_per_day, ynums)
        plt.ylabel('RH (MAGIC) - RH (NOT)')
        plt.savefig('{:s}/Humidity_corr_NOT_time{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

        plt.figure(figsize = (10,5), constrained_layout = True)
        not_profile_time(df_not[df_not['Humidity']<90],dff[dff[name_humidity]<90],'Humidity',name_humidity,expected_data_per_day,ynums,85,print_threshold=50)
        plt.ylabel('RH (MAGIC) - RH (NOT)')
        plt.savefig('{:s}/Humidity_corr_NOT_time_lowhum{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

        plt.figure(figsize = (10,5), constrained_layout = True)
        not_profile_time(df_not,dff[dff['temperature_reliable']==True],'TempInAirDegC',name_temperature,expected_data_per_day,ynums,85,print_threshold=10)
        plt.ylabel('Temp (MAGIC) - Temp (NOT)')
        plt.savefig('{:s}/Temperature_corr_NOT_time{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')
    ...
    if has_multiple_data_spacings:        
        mask_before = ((dfff.index < config['WS_dataspacing_change']) & (dfff[name_temperature]>config['minimum_humidity']) & (dfff[name_temperature]<config['maximum_humidity']))
        mask_after = ((dfff.index > config['WS_dataspacing_change']) & (dfff[name_temperature]>config['minimum_humidity']) & (dfff[name_temperature]<config['maximum_humidity']))

        dfff_before, coverage_before = apply_coverage(dfff[mask_before],data_spacing=config['data_spacing_before'],debug=False)
        dfff_after, coverage_after = apply_coverage(dfff[mask_after],data_spacing=config['data_spacing_after'],debug=False)
    ...
    if (is_20to22):
        dfff_no, coverage_no = apply_coverage(dfff_no,data_spacing_minutes,debug=False)
    ...
    init_hum = config['init_hum']
    init_mu = eval(init_hum, {"np": np})
    ...
    init_hum2 = config['init_hum2']
    init_mu2 = eval(init_hum2, {"np": np})    
    ...
    init_hum4 = config['init_hum4']
    init_mu4 = eval(init_hum4, {"np": np})    
    ...
    if is_offset:
        init_hum2_offset = config['init_hum2_offset']
        init_mu2_offset = eval(init_hum2_offset, {"np": np})        
    ...
    if (is_20to22):
        ...
        hum_lik_median4_no.setargs_df(dfn_no,name_humidity,mask_no,
                                      is_daily=True,is_median=True,day_coverage=day_coverage_for_humidity,
                                      data_expected=expected_data_per_day)
        hum_lik_median4_no.like_minimize(method=method,tol=tol)

    init_hum_month = config['init_hum_month']
    init_month = eval(init_hum_month, {"np": np})        
    ...
    plt.savefig('{:s}/Humitat_residuals{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('{:s}/Humitat_residuals2{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    a[1].legend(loc='best')    
    plt.savefig('{:s}/Humitat_residuals_hist{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    ...
    if is_offset:
        ...
        a[1].legend(loc='best')    
        plt.savefig('{:s}/Humitat_residuals_hist_offset{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    ...
    plt.savefig('{:s}/Humitat_b_profiled{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    tits = tits.replace('_offset','')
    tits = tits.replace('_coauthors','')
    ...
    #plt.xlim(0,100)
    plt.savefig('{:s}/Histogram_humidity{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()

    plt.clf()
    plt.plot(dfff.index, dfff[name_humidity])
    plt.ylabel('Relative humidity (%)')
    plt.savefig('{:s}/Histogram_lowhumidity_vsTime{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.ylabel('Probability / %')
    #plt.xlim(0,100)
    plt.savefig('{:s}/Histogram_loghumidity{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_hist(dfff, name_humidity, 1.,'Relative humidity (%)','Probability / %', '\%', xoff=0., loc='right', coverage_cut=coverage_cut)
    #plt.plot(x,y, color= 'steelblue')
    plt.savefig('{:s}/Hist_humidity{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_hist(dfff, name_humidity, 1.,'Relative humidity (%)','Probability / %', '\%', xoff=0., loc='right', is_night=True, coverage_cut=coverage_cut)
    #plt.plot(x,y, color= 'steelblue')
    plt.savefig('{:s}/Hist_humidity_night{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
 
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_mensual(dfff, name_humidity,'RH (%)', fullfits=True)
    plt.savefig('{:s}/Humitat_mensual_fullfits{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_mensual(dfff, name_humidity,'Relative humidity (%)')
    plt.savefig('{:s}/Humitat_mensual{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_mensual(dfn, name_humidity,'Relative humidity (%)')
    plt.savefig('{:s}/Humitat_mensual_low{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_mensual_diurnal(dfn, name_humidity,r'$\Delta$ RH (%)',expected_data_per_day, min_coverage=coverage_cut, day_coverage=day_coverage_for_samples, is_lombardi=True)
    plt.ylim([-20,20])
    plt.savefig('{:s}/Humitat_mensual_diurnal{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
        
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_sunalt(dfn[dfn[name_humidity]<90], name_humidity,r'Median relative humidity (%)', join_months=True)
    plt.ylim([10., 50.])
    plt.savefig('{:s}/Humitat_diurnal{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_mensual_distributions(dfn, name_humidity, 'Relative Humidity - monthly mean (ºC)', 0.1)
    plt.savefig('{:s}/Humidity_distributions{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_historic(dfff, name_humidity, coverage, ynums)
    plt.ylabel('Relative Humidity (%)')
    #plt.xlabel('Year')
    plt.savefig('{:s}/Humitat_sencer{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.ylabel('Relative Humidity (%)')
    #plt.xlabel('Year')
    #plot_historic_fit_results(dfn,mask,hum_lik_median,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_samples)
    #plot_historic_fit_results(dfn,mask,hum_lik_median4,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_samples,color='orange',is_sigma2=True)
    if is_offset:
        plot_historic_fit_results(dfn,mask1,hum_lik_median4_offset,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_humidity,
                                  color='red',is_sigma2=True,is_offset=True)
        plot_historic_fit_results(dfn,mask2,hum_lik_median4_offset,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_humidity,
                                  color='orange',is_sigma2=True,is_offset=True,offset=hum_lik_median4_offset.res.x[-1])
        tits = tits + '_offset'        
    else:
        plot_historic_fit_results(dfn,mask,hum_lik_median4,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_humidity,color='red',is_sigma2=True)
    plt.savefig('{:s}/Humitat_sencer_fits{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    tits = tits.replace('_offset','')
    ...
    for i in gradients:
        ...
        #plt.plot(x,y, color= 'steelblue')
        plt.savefig(plotdir+'/Hist_'+var+'{:s}.pdf'.format(tits), bbox_inches='tight')
        plt.show()

        plt.clf()
        plot_hist(dfff, var, 0.5,'Humidity change rate (%/min)','Probability / (%/min)', '%/min', xoff=0.08, is_night=True, coverage_cut=coverage_cut)
        #plt.plot(x,y, color= 'steelblue')
        plt.savefig(plotdir+'/Hist_'+var+'_night{:s}.pdf'.format(tits), bbox_inches='tight')
        plt.show()
        
        plt.clf()
        plot_mensual(dfff, var, 'Humidity change rate (%/min)')
        plt.savefig(plotdir+'/'+var+'_mensual{:s}.pdf'.format(tits), bbox_inches='tight')
        plt.show()
    
        plt.figure(figsize = (10,5), constrained_layout = True)
        plot_historic(dfff, var, coverage, ynums)
        #plt.xlabel('Year')
        plt.ylabel('Humidity change rate (%/min)')
        plt.savefig(plotdir+'/'+var+'_sencer{:s}.pdf'.format(tits), bbox_inches='tight')
        plt.show()
    ...
    plt.savefig('{:s}/Tgradient1_vs_Rgradient1{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Humidity change rate (%/min)')
    plt.ylabel('Temperature change rate (ºC/min)')
    plt.savefig('{:s}/Tgradient1_vs_Rgradient1_storm{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Humidity change rate (%/min)')
    plt.ylabel('Temperature change rate (ºC/min)')
    plt.savefig('{:s}/Tgradient1_vs_Rgradient1_nostorm{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Humidity change rate (%/min)')
    plt.ylabel('Temperature change rate (ºC/min)')
    plt.savefig('{:s}/Tgradient5_vs_Rgradient5{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Humidity change rate (%/min)')
    plt.ylabel('Temperature change rate (ºC/min)')
    plt.savefig('{:s}/Tgradient5_vs_Rgradient5_storm{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Humidity change rate (%/min)')
    plt.ylabel('Temperature change rate (ºC/min)')
    plt.savefig('{:s}/Tgradient5_vs_Rgradient5_nostorm{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('{:s}/TemperatureDP_vs_Humidity_baddata{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('{:s}/Temperature_vs_DP_highHum_baddata{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
...
def plot_pressure() -> None:
    ...
    num=100
    #print ('PGRADIENT0: ',dff['Pgradient0'].head(n=num))
    #print ('PGRADIENT0: ',dff['diff0'].head(n=num))
    print ('High pressure: ',dff.loc[ dff[name_pressure] > 780,name_pressure].head(n=20))
    print ('Low pressure: ',dff.loc[ dff[name_pressure] < 720,name_pressure].head(n=20))    
    
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dff[name_pressure],dff['Pgradient0'],nbins=150)
    plt.xlabel('Pressure (hPa)', fontsize=22)
    plt.ylabel('Pressure gradient',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Pgradient0_vs_Pressure{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dff['Pgradient0'],dff[name_pressure],nbins=150)
    plt.ylabel('Pressure (hPa)', fontsize=22)
    plt.xlabel('Pressure gradient',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Pressure_vs_Pgradient0{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dff['Tgradient0'],dff['Pgradient0'],nbins=150)
    plt.xlabel('Temperature gradient', fontsize=22)
    plt.ylabel('Pressure gradient',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Pgradient0_vs_Tgradient0{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dff['Pgradient0'],dff['Tgradient0'],nbins=150)
    plt.ylabel('Temperature gradient', fontsize=22)
    plt.xlabel('Pressure gradient',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Tgradient0_vs_Pgradient0{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    dfff = dff[((dff['pressure_reliable']==True) & (dff['Pgradient0']<2) & (dff[name_pressure]<config['maximum_pressure']) & (dff[name_pressure]>config['minimum_pressure']) & (dff['Pgradient0']>-2))]
    dfff_h = dff[(((dff['pressure_reliable']==True) & (dff['Pgradient0']<2)  & (dff[name_pressure]<config['maximum_pressure']) & (dff[name_pressure]>config['minimum_pressure']) & (dff['Pgradient0']>-2)) | (dff.index < new_WS))]
    #dfff_h = dff[(((dff['pressure_reliable']==True) & (dff['Pgradient0']<1.09)  & (dff[name_pressure]<config['maximum_pressure'])  & (dff['Pgradient1']<25) & (dff['Pgradient0']>-1.09)) | (dff.index < new_WS))]

    #dfff = dff[((dff['pressure_reliable']==True) & (dff[name_pressure]<config['maximum_pressure']))]
    #dfff_h = dff[(((dff['pressure_reliable']==True)  & (dff[name_pressure]<config['maximum_pressure'])) | (dff.index < new_WS))]

    print ('High pressure: ',dfff.loc[ dfff[name_pressure] > config['maximum_pressure'],name_pressure].head(n=20))
    
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dfff[name_pressure],dfff['Pgradient0'],nbins=150)
    plt.xlabel('Pressure (hPa)', fontsize=22)
    plt.ylabel('Pressure gradient',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Pgradient0_r_vs_Pressure{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dfff['Pgradient0'],dfff[name_pressure],nbins=150)
    plt.ylabel('Pressure (hPa)', fontsize=22)
    plt.xlabel('Pressure gradient',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Pressure_vs_Pgradient0_r_{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dfff[name_pressure],dfff['Pgradient1'],nbins=50)
    plt.xlabel('Pressure (hPa)', fontsize=22)
    plt.ylabel('Pressure gradient',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Pgradient1_vs_Pressure{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dfff['Pgradient1'],dfff[name_pressure],nbins=50)
    plt.ylabel('Pressure (hPa)', fontsize=22)
    plt.xlabel('Pressure gradient',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Pressure_vs_Pgradient1{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    dfff, coverage = apply_coverage(dfff,data_spacing_minutes,debug=False)
    dfff_h, coverage_h = apply_coverage(dfff_h,data_spacing_minutes,debug=False)
    dfn = dfff[(dfff['coverage'] > coverage_cut_for_daily_samples)]

    day_coverage_for_pressure = config['day_coverage_for_pressure']
    minimum_pressure = config['minimum_pressure']
    maximum_pressure = config['maximum_pressure']

    
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dfff[name_pressure],dfff[name_temperature],nbins=50)
    plt.xlabel('Pressure (hPa)', fontsize=22)
    plt.ylabel('Temperature (ºC)',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Temperature_vs_Pressure{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dfff[name_temperature],dfff[name_pressure],nbins=50)
    plt.ylabel('Pressure (hPa)', fontsize=22)
    plt.xlabel('Temperature (ºC)',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Pressure_vs_Temperature{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dfff[name_humidity], dfff[name_pressure],nbins=50)
    plt.ylabel('Pressure (hPa)', fontsize=22)
    plt.xlabel('Relative Humidity (%)',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Pressure_vs_Humidity{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dfff[name_pressure], dfff[name_humidity],nbins=50)
    plt.xlabel('Pressure (hPa)', fontsize=22)
    plt.ylabel('Relative Humidity (%)',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Humidity_vs_Pressure{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')



    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dfff['Pgradient1'],dfff[name_temperature],nbins=50)
    plt.xlabel('Pgradient1 (hPa)', fontsize=22)
    plt.ylabel('Temperature (ºC)',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Temperature_vs_Pgradient1{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dfff[name_temperature],dfff['Pgradient1'],nbins=50)
    plt.ylabel('Pgradient1 (hPa)', fontsize=22)
    plt.xlabel('Temperature (ºC)',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Pgradient1_vs_Temperature{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dfff[name_humidity], dfff['Pgradient1'],nbins=50)
    plt.ylabel('Pgradient1 (hPa)', fontsize=22)
    plt.xlabel('Relative Humidity (%)',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Pgradient1_vs_Humidity{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_profile(dfff['Pgradient1'], dfff[name_humidity],nbins=50)
    plt.xlabel('Pgradient1 (hPa)', fontsize=22)
    plt.ylabel('Relative Humidity (%)',fontsize=22)
    plt.legend(loc='best')
    plt.savefig('{:s}/Humidity_vs_Pgradient1{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')



    mask  = ((dfn.index > new_WS) & (dfn[name_pressure]>minimum_pressure)) 
    mask1 = ((dfn.index > new_WS) & ((dfn.index < new_model) | (dfn.index > old_model))  & (dfn[name_pressure]>minimum_pressure))
    mask2 = ((dfn.index > new_model) & (dfn.index < old_model) & (dfn[name_pressure]>minimum_pressure))  
    #mask = ((dfn.index > WS_relocation)  & (dfn[name_pressure]>750))    

    plt.clf()
    plot_mensual(dfff, name_pressure, 'Atmospheric pressure (hPa)', coverage_cut=coverage_cut, fullfits=True)
    if tits == "_Paranal":
        plt.ylim([742.,745.])
    plt.savefig('{:s}/Pressure_mensual{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()

    dfn_H = dfff[(dfff['coverage'] > coverage_cut_for_monthly_samples)]
    mask_H = ((dfn_H.index > new_WS)  & (dfn_H[name_pressure]>minimum_pressure))
    
    init_month_pressure = config['init_month_pressure']
    init_month = eval(init_month_pressure, {"np": np})
    ...
    init_mu2_pressure = config['init_mu2_pressure']
    init_mu2 = eval(init_mu2_pressure, {"np": np})    
    ...
    init_mu2_sig2_pressure = config['init_mu2_sig2_pressure']
    init_mu2_sig2 = eval(init_mu2_sig2_pressure, {"np": np})
    ...
    init_mu2_offset_pressure = config['init_mu2_offset_pressure']
    init_mu2_offset = eval(init_mu2_offset_pressure, {"np": np})
    ...
    init_mu2_sig2_offset_pressure = config['init_mu2_sig2_offset_pressure']
    init_mu2_sig2_offset = eval(init_mu2_sig2_offset_pressure, {"np": np})        
    ...
    plt.savefig('{:s}/Pressure_residuals{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('{:s}/Pressure_residuals2{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('{:s}/Pressure_residuals4_offset{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    a[1].text(6,1000.,r'$\chi^{2}/NDF$='+'{:.2f}'.format(chi2_measured_median4),fontsize=20)
    a[1].legend(loc='best')    
    plt.savefig('{:s}/Pressure_residuals_hist{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    ...
    a[1].text(6,1000.,r'$\chi^{2}/NDF$='+'{:.2f}'.format(chi2_measured_median4_offset),fontsize=20)
    a[1].legend(loc='best')    
    plt.savefig('{:s}/Pressure_residuals_hist_offset{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    ...
    if is_naoi:
        ...
        plt.savefig('{:s}/Pressure_corr_d_NAOI{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')
    ...
    plt.savefig('{:s}/Pressure_b_profiled{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    ...
    plt.savefig('{:s}/Histogram_pressure{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.xticks(fontsize = 25)
    plt.yticks(fontsize = 25)
    plt.savefig('{:s}/Histogram_logpressure{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()

    plt.clf()
    plot_hist(dfff, name_pressure, 0.25,'Atmospheric Pressure (hPa)','Probability / hPa', ' hPa',xoff=0., coverage_cut=coverage_cut)
    #plt.plot(x,y, color= 'steelblue')
    plt.savefig('{:s}/Hist_pressure{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    
    plt.clf()
    plot_hist(dfff, name_pressure, 0.25,'Atmospheric Pressure (hPa)','Probability / hPa', ' hPa',xoff=0., is_night=True, coverage_cut=coverage_cut)
    #plt.plot(x,y, color= 'steelblue')
    plt.savefig('{:s}/Hist_pressure_night{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()

    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_historic(dfff_h, name_pressure, coverage_h, ynums)
    plt.ylabel('Atmospheric pressure (hPa)')
    #plt.xlabel('Year')
    #plot_historic_fit_results(dfn,mask,press_lik_median2,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_pressure,color='red',is_sigma2=False)
    #plot_historic_fit_results(dfn,mask,press_lik_median4,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_pressure,color='red',is_sigma2=True)
    plot_historic_fit_results(dfn,mask1,press_lik_median2_offset,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_pressure,
                              color='red',is_sigma2=False,is_offset=True)
    plot_historic_fit_results(dfn,mask2,press_lik_median2_offset,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_pressure,
                              color='red',is_sigma2=False,is_offset=True,offset=press_lik_median2_offset.res.x[-1])
    #plot_historic_fit_results(dfn,mask1,press_lik_median41,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_pressure,color='tomato',is_sigma2=True)
    #plot_historic_fit_results(dfn,mask2,press_lik_median42,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_pressure,color='seagreen',is_sigma2=True)            
    plt.savefig('{:s}/Pressure_sencer{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.ylabel('Temperature change rate (ºC/min)')
    plt.savefig('{:s}/Tgradient_vs_Pgradient{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Pressure change rate (hPa/min)')
    plt.ylabel('Temperature change rate (ºC/min)')
    plt.savefig('{:s}/Tgradient1_vs_Pgradient1_storm{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Pressure change rate (hPa/min)')
    plt.ylabel('Temperature change rate (ºC/min)')
    plt.savefig('{:s}/Tgradient1_vs_Pgradient1_nostorm{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Pressure change rate (hPa/min)')
    plt.ylabel('Temperature change rate (ºC/min)')
    plt.savefig('{:s}/Tgradient5_vs_Pgradient5{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Pressure change rate (hPa/min)')
    plt.ylabel('Temperature change rate (ºC/min)')
    plt.savefig('{:s}/Tgradient5_vs_Pgradient5_storm{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Pressure change rate (hPa/min)')
    plt.ylabel('Temperature change rate (ºC/min)')
    plt.savefig('{:s}/Tgradient5_vs_Pgradient5_nostorm{:s}.png'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
...
def plot_wind() -> None:
    ...
    plt.vlines(v50,1e-4,5e-3,colors='cadetblue')
    plt.vlines(v475,1e-4,5e-4,colors='darkslategrey')
    plt.hlines(2e-2,v10,v10+10,colors='darkturquoise')
    plt.hlines(5e-3,v50,v50+10,colors='cadetblue')
    plt.hlines(5e-4,v475,v475+10,colors='darkslategrey')
    ...
    if correlate_ctaos:
        df_ctaos = pd.read_hdf(ctaos_file)
        print ('CTAOS: ', df_ctaos.head())
        mask_ctaos = ((dfff.index > CTAOS_start_date) & (dfff.index < CTAOS_end_date))
        ...
        plt.ylabel('Wind Gust (Paranal) - Wind Gust (CTAO-S) (km/h)')
        plt.savefig('{:s}/WindGust_corr_CTAO-S_time{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')    
    ...
    plt.savefig('{:s}/WindDirectionAverage{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()
    ...
    if is_magic:
        ...
        plot_mensual_wind(dfff,fullfits=True,
                          name_ws_average=name_ws_average,name_ws_current=name_ws_current,name_ws_gust=name_ws_gust,
                          ylim_outliers=[100.,160.], ylim_bulk=[0.,50.])
    #plt.xlabel('Month')
    ...
    plt.legend(loc='best')
    plt.savefig('{:s}/WindTI_corr_WindSpeed{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')
    ...
    plt.xlim([0.,360.])
    plt.legend(loc='best')
    plt.savefig('{:s}/WindTInorm_corr_WindDir{:s}.pdf'.format(plotdir,tits),bbox_inches='tight')
    ...
    plt.savefig('{:s}/WindTI_corr_WindDir{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    ...
    plt.xlabel('Wind Speed Average (km/h)')
    plt.ylabel('Wind Direction',fontsize=22)
    plt.savefig('{:s}/WindDir_corr_WindSpeed{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
...
v    #plt.ylabel('Wind Speed (km/h)')
    ...
    plt.figure(figsize = (10,5), constrained_layout = True)
    plot_historic(dfff[mask_ti], 'windTI', coverage, ynums)
    #plt.xlabel('Year')
    plt.ylabel('Turbulence Index')
    plt.savefig('{:s}/WindTI_sencer{:s}.pdf'.format(plotdir,tits), bbox_inches='tight')
    plt.show()

    return
    
    day_coverage_for_windSpeedAverage = 70
    alisio_limit = 50   # MAGIC 
    alisio_limit = 105  # Paranal
    ...
    if is_offset:
        plot_historic_fit_results(dfn,mask1,wind_lik_mean_offset,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_windSpeedAverage,
                                  color='red',is_sigma2=False,is_offset=True)
        plot_historic_fit_results(dfn,mask2,wind_lik_mean_offset,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_windSpeedAverage,
                                  color='orange',is_sigma2=False,is_offset=True,offset=wind_lik_mean_offset.res.x[-1])
        tits = tits + '_offset'        
    else:
        plot_historic_fit_results(dfn,mask,wind_lik_median4,expected_data_per_day,is_daily=True, day_coverage=day_coverage_for_windSpeedAverage,color='red',is_sigma2=True)    
    #plt.xlabel('Year')
    plt.ylabel('Wind Speed (km/h)')
...
if __name__ == '__main__':
    ...
    print('LOW HUMIDITY: ',dff[dff[name_humidity]<0.1])
    print('MJD not valid: ',dff[dff['mjd'].isnull()])
    #dff = dff[dff['is_dup']==False]

    if config_file == 'configs/not.yaml':
        dff = dff[dff['Tgradient1']<3]
